...
namespace = Collection("nautobot_ipfdemo")
namespace.configure(
    {
        "nautobot_ipfdemo": {
            "project_name": "nautobot-ipfdemo",
            "python_ver": "3.8",
            "local": False,
...
def docker_compose(context, command, **kwargs):
    ...
    """
    compose_env = {"PYTHON_VER": context.nautobot_ipfdemo.python_ver, "NAUTOBOT_VERSION": NAUTOBOT_VERSION}
    compose_command = f'docker-compose --project-name {context.nautobot_ipfdemo.project_name} --project-directory "{context.nautobot_ipfdemo.compose_dir}"'
    for compose_file in context.nautobot_ipfdemo.compose_files:
        compose_file_path = os.path.join(context.nautobot_ipfdemo.compose_dir, compose_file)
...
def run_command(context, command, **kwargs):
    ...
    if is_truthy(context.nautobot_ipfdemo.local):
        context.run(command)
    else:
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot {NAUTOBOT_VERSION} with Python {context.nautobot_ipfdemo.python_ver}...")
...
def nbshell(context):
    ...
    if context.nautobot_ipfdemo.use_django_extensions:
...
def import_nautobot_data(context):
    ...
    context.nautobot_ipfdemo.local = False
    copy_cmd = f"docker cp nautobot_data.json {context.nautobot_ipfdemo.project_name}_nautobot_1:/tmp/nautobot_data.json"
...
def db_export(context):
    ...
    copy_cmd = f"docker cp {context.nautobot_ipfdemo.project_name}_postgres_1:/tmp/nautobot.sql dev/nautobot.sql"
...
def db_import(context):
    ...
    copy_cmd = f"docker cp dev/nautobot.sql {context.nautobot_ipfdemo.project_name}_postgres_1:/tmp/nautobot.sql"
    context.run(copy_cmd)
...
def unittest(context, keepdb=False, label="nautobot_ipfdemo_plugin", failfast=False):
...
def tests(context, lint_only=False):
    ...
    if not is_truthy(context.nautobot_ipfdemo.local):
        start(context)
    print("Running black...")
    black(context)
